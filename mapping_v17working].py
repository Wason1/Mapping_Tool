...
def prepare_match_data(application, s1, s2, progressbar, threshold=70):
    s2_with_indices = [(i, elem) for i, elem in enumerate(s2)]
    length = len(s1)
...
class Application:
    ...
    def match_data(self):
        if self.spreadsheet1 is not None and self.spreadsheet2 is not None and self.column1 is not None and self.column2 is not None:
            prepare_match_data(self, self.spreadsheet1[self.column1].values, self.spreadsheet2[self.column2].values, self.progressbar)
